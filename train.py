...
# Mask the inputs
mask = Mask(f6=True, top=False, fw_and_l=False, fwl=True, strips_param=True, roll_param=True, hardness_indic=True, family=True, suppliers=True, cum_length=False)
x_train = mask.apply(x_train)
x_test = mask.apply(x_test)
x_valid = mask.apply(x_valid)
...
# Model
model = pytorch_model.LinearModel().to(device)
